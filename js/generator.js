...
const Generator = (() => {
  ...
  /* ----------------------------------------------------------------
     ...
     esvaziados, para evitar que treinos de uma geração anterior (ou
     de testes/edições manuais) "sobrem" e apareçam como se tivessem
     sido gerados agora.
  ---------------------------------------------------------------- */
  async function generateAndSave(params) {
    ...
    // Esvazia todos os dias da semana que não fazem parte da seleção atual
    for (const dayCode of WEEKDAY_ORDER) {
      if (!selectedSet.has(dayCode)) {
        await window.DB.saveWorkoutDay(dayCode, '', []);
      }
    }
